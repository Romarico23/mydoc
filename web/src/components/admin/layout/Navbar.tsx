...
function Navbar() {
	...
	const {
		...
		queryFn: () => viewAdminNotificationList(axiosAuth),
	...
	return (
		<div className="bg-primary-foreground dark:bg-slate-700 py-2 pl-3 pr-5 flex justify-between items-center shadow-md">
			<Link href="/">
							...
							viewNotification?.notifications?.map(
								(notification: NotificationData) => (
									<DropdownMenuItem
										...
										onClick={() =>
											router.push(
												`/admin/appointments/${notification.appointmentId}`
											)
										}
									>
